    ...
    (set, get) => ({
      ...
      fetchUser: async () => {
        try {
          ...
          const response = await fetchWithAuth(
            `${import.meta.env.VITE_API_BASE_URL || 'https://new-peer-1.onrender.com'}/users/profile`,
            { credentials: 'include' }
          );
          ...
          const friendsResponse = await fetchWithAuth(
      ...
      sendFriendRequest: async (targetUserId) => {
        try {
          set({ isLoading: true, error: null });
          const response = await fetchWithAuth(
            `${import.meta.env.VITE_API_BASE_URL || 'https://new-peer-1.onrender.com'}/users/friend-request`,
            {
              method: 'POST',
      ...
      acceptFriendRequest: async (requesterId) => {
        try {
          set({ isLoading: true, error: null });
          const response = await fetchWithAuth(
            `${import.meta.env.VITE_API_BASE_URL || 'https://new-peer-1.onrender.com'}/users/friend-request/accept`,
            {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              credentials: 'include',
          ...
          const friendResponse = await fetchWithAuth(
            `${import.meta.env.VITE_API_BASE_URL || 'https://new-peer-1.onrender.com'}/users/${requesterId}`,
            { credentials: 'include' }
          );
      ...
      rejectFriendRequest: async (requesterId) => {
        try {
          set({ isLoading: true, error: null });
          const response = await fetchWithAuth(
            `${import.meta.env.VITE_API_BASE_URL || 'https://new-peer-1.onrender.com'}/users/friend-request/reject`,
            {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              credentials: 'include',
      ...
      addFriend: async (friendId) => {
        try {
          ...
          const response = await fetchWithAuth(
            `${import.meta.env.VITE_API_BASE_URL || 'https://new-peer-1.onrender.com'}/users/add-friend`,
            {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
              credentials: 'include',
          ...
          const friendResponse = await fetchWithAuth(
            `${import.meta.env.VITE_API_BASE_URL || 'https://new-peer-1.onrender.com'}/users/${friendId}`,
            { credentials: 'include' }
          );
      ...
      removeFriend: async (friendId) => {
        try {
          ...
          if (!user) throw new Error('Authentication required');
          const response = await fetchWithAuth(
            `${import.meta.env.VITE_API_BASE_URL || 'https://new-peer-1.onrender.com'}/users/remove-friend`,
            {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
              credentials: 'include',
      ...
      fetchBadges: async () => {
        try {
          set({ isLoading: true, error: null });
          const response = await fetchWithAuth(
            `${import.meta.env.VITE_API_BASE_URL || 'https://new-peer-1.onrender.com'}/users/get-badge`,
            { credentials: 'include' }
          );
          if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
